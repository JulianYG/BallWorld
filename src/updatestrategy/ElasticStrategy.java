...
public class ElasticStrategy implements IUpdateStrategy<IBallCmd> {
			...
			public void apply(Ball other, IDispatcher<IBallCmd> disp) {
				if (context != other) {
					if (context.getLocation().distance(other.getLocation()) <= (context.getRadius() + other.getRadius())) {
						...
						Point2D.Double impulse = impulse(context.getLocation(),

								context.getVelocity(), other.getLocation(), other.getVelocity(), reducedMass(context.getMass(), other.getMass()),

								context.getLocation().distance(other.getLocation()), (context.getRadius() + other.getRadius() - (context.getLocation().distance(other.getLocation()))));
						updateCollision(context, other, impulse.getX(), impulse.getY(), disp); 
						updateCollision(other, context, -impulse.getX(), -impulse.getY(), disp);
					}
				}
			}
		});
	 ...
	 * @return the reduced mass of two balls
	 */
	 ...
	 * @return the impulse (change in momentum) of the collision in the direction from the source to the target
	 */
